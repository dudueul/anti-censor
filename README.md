...
- **Right-click any video → “Obfuscate video (anti-censor)”** → per-frame
  transform + temporal restructuring (video), plus a pitch-micro-shift + noise on
  the **audio** track, re-encoded to WebM (VP8 + Opus). Runs in an offscreen
  document (WebCodecs); best-effort and suited to short clips.
